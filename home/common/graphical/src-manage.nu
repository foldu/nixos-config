...
def "main import" [path: path] {
  ...
  let target_path = (repo-path-from-origin $origin)
...
def repo-path-from-origin [origin: string] {
  let repo_info = try {
    repo-info-from-url $origin
  } catch {
    repo-info-from-ssh $origin
  }
  repo-target-path $repo_info
}

def repo-target-path [repo_info] {
  let flatten = try {
...
def config-parse [] {
  ...
  open $"($config_dir)/src-manage/config.yml"
